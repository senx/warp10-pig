...
public class ConvertToLepton extends EvalFunc<DataBag> {
      ...
      if (null != valuesBag) {
        ...
        while (iter.hasNext()) {
          ...
          if (encoder.size() > this.threshold || !iter.hasNext()) {
            ...
            GTSWrapper wrapper = toGTSWrapper(encoder, metadata);
  ...
  protected Tuple decodeValues(final Tuple input) throws IOException {
    ...
    if (input.size() < 2 || input.size() > 5) {
      throw new IOException(
          "Invalid input, tuples should have 2, 3, 4, or 5 elements.");
    }
  ...
  /**
    * Convert GTSEncoder to GTSWrapper
    */
  protected GTSWrapper toGTSWrapper(final GTSEncoder encoder, Metadata metadata) {
   ...
   *
   * @param input
   * @return gts: (gts rawData: {bag: {t: (gts: bytearray)}}
   */
  @Override
